...
We will see a quick setup to configure the ``nsd`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``nsd`` formula
`````````````````````````````````````````````````````

Now, we can provides the per DNS domain name configuration files for the ``nsd`` formulas under ``/srv/salt/nsd/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/nsd/parameters/dns:domain/
    ln -s dns:domain /srv/salt/nsd/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/nsd/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/nsd/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``nsd`` formula
``````````````````````````````````````````````````````

Now, we can provides the per role configuration files for the ``nsd`` formulas under ``/srv/salt/nsd/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/nsd/parameters/roles
...
We create a configuration for the role ``nsd/server`` in ``/srv/salt/nsd/parameters/roles/nsd/server.yaml``:
...
We create another configuration for the role ``nsd/client`` in ``/srv/salt/nsd/parameters/roles/nsd/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles nsd/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles nsd/client
...
.. code-block:: console
    ...
    mv /srv/salt/nsd/parameters/dns:domain/  '/srv/salt/nsd/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-nsd-as-json:
      test.nop:
        - name: {{ nsd | json }}
...
Here is an example based on `template-formula/nsd/config/file.sls`_:
...
This ``sls`` file expose a ``nsd`` context variable to the jinja template which could be used like this:
...
.. _template-formula/nsd/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/nsd/config/file.sls
